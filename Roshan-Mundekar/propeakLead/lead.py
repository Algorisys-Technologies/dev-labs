...
PRODUCT_PATTERNS = [
    ...
    r"(?:Aquapetal|Cosmetic Beads)",  # These seem like hardcoded product mentions
    r"(?:price|cost)\s+for\s+.*?\s+of\s+(?P<prod>[^\n]+)",
    r"(?:We|I)\s+(?:want|am looking|are looking)\s+to\s+(?:buy|purchase|order|enquire)(?:\s+about)?\s+(?P<prod>[^\n]+)"
]

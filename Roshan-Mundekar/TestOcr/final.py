...
def refine_with_gpt(text, prompt=None):
    ...
    try:
        if not prompt:
            ...
            # 5. Remove junk, random alphanumeric strings (like "33385afH", "PIC", "3PIC","H382", repeated "EPIC"), and meaningless words.
            ...
            # 8. Use the label 'ID:' uniformly for voter ID numbers (e.g., "UGU1996123", "IQZ0353417") instead of 'STATE:' or 'DISTRICT:' if it's actually a unique ID.
            ...
            4. Output clean text only
            5. Remove junk, random alphanumeric strings (like "33385afH", "PIC", "3PIC","H382", repeated "EPIC"), and meaningless words.
            6. **Do NOT modify names, ID numbers, or codes** (e.g., "TULSI TETYAL", "UGU1996123"). Preserve them **exactly as-is**, including spacing or capitalization.
            7. Preserve all valid information from the OCR exactly as-is unless clearly broken.
            8. Use the label 'ID:' uniformly for voter ID numbers (e.g., "UGU1996123", "IQZ0353417") instead of 'STATE:' or 'DISTRICT:' if it's actually a unique ID.
            9. Fix spacing in names like "Sivarajan.VN" ➝ "Sivarajan V. N", **but never change spelling or rearrange letters**.

...
export const loader = async () => {
  ...
  const emails = await prisma.enron_emaildataset.findMany();
      ...
      <div className="flex flex-1">
        <aside className="w-1/4 bg-gray-200 p-4">
          <div className="space-y-4">
            <h2 className="font-bold text-xl">📨 YOUR EMAILS ACOUNTS</h2>
            {accounts.map((account) => (
              <button
                key={account.id}
                className={`w-full text-left p-2 border rounded-md hover:bg-black-100 focus:outline-none ${selectedAccount?.id === account.id
                  ? "bg-black text-white"
                  : "bg-white"
                  }`}
            ...
            <Link
              to="/search-form"
              className="flex items-center w-full mt-5 text-left p-2 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none"
            >
              <MagnifyingGlassIcon className="w-5 h-5 mr-2 text-gray-500" />
              <span>SEARCH EMAIL ENGINE NLP</span>
            </Link>

defmodule Ignite.DebugPage do
  def render_error(conn, error, stack) do
    # Only show the debug page in development
    if Application.get_env(:ignite, :env) == :dev do
      render_debug_page(conn, error, stack)
    else
      render_prod_page(conn)
    end
  end

  defp render_prod_page(_conn) do
    """
    <!DOCTYPE html>
    <html>
  ...
  defp render_debug_page(exception, stacktrace, conn) do
    ...
    <head>
      ...
      <style>
        ...
        header pre { background: rgba(0,0,0,0.2); padding: 12px; border-radius: 6px; white-space: pre-wrap; word-break: break-all; }
    ...
    <body>
      ...
      <nav id="tabs">
        <button class="tab active" onclick="showTab('stacktrace', event)">Stacktrace</button>
        <button class="tab" onclick="showTab('request', event)">Request</button>
        <button class="tab" onclick="showTab('session', event)">Session</button>
      </nav>

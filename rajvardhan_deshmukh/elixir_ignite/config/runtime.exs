...
if config_env() == :prod do
  ...
  config :elixir_ignite, port: port
  ...
  config :elixir_ignite, secret_key_base: secret_key_base
  ...
  case System.get_env("HTTP_REDIRECT_PORT") do
    nil -> :ok
    val -> config(:elixir_ignite, http_redirect_port: String.to_integer(val))
  end
  ...
  case System.get_env("RATE_LIMIT_MAX") do
    ...
    val ->
      ...
      config :elixir_ignite, rate_limit: [max_requests: String.to_integer(val), window_ms: window]
  end
end

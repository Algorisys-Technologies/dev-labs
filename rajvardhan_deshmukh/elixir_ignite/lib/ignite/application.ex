defmodule Ignite.Application do
  ...
  def start(_type, _args) do
    # We pull the port from config, defaulting to 4000.
    # In config/test.exs we'll set this to 4002.
    port = Application.get_env(:elixir_ignite, :port, 4000)
    ...
    children = [
      ...
      Ignite.Presence, # Must start after PubSub
      Ignite.RateLimiter,
  ...
  # Set `config :elixir_ignite, http_redirect_port: 4080` to enable.
  defp redirect_children(https_port) do
    http_port = Application.get_env(:elixir_ignite, :http_redirect_port)
  ...
  defp dev_children do
    if Application.get_env(:elixir_ignite, :env) == :dev do
      [Ignite.Reloader]
    else
      []
    end

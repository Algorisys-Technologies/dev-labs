defmodule Ignite.RateLimiter do
  @moduledoc """
  ETS-based sliding window rate limiter for Ignite.
  """
  use GenServer
  require Logger
  ...
  """
  def call(conn) do
    config = Application.get_env(:elixir_ignite, :rate_limit, [])
  ...
  defp client_ip(conn) do
    ...
    case Map.get(conn.req_headers, "x-forwarded-for") do
      nil ->
  ...
  defp add_rate_limit_headers(%Ignite.Conn{} = conn, limit, remaining, reset_unix) do
    new_headers =
      conn.resp_headers
  ...
  defp add_resp_header(%Ignite.Conn{} = conn, key, value) do

...
// after resolving symbolic links when possible.
...
// ValidateInputPath checks that path exists and is readable.
func ValidateInputPath(path string) error {
	...
	if strings.ToLower(filepath.Ext(path)) != ".csv" {
		return fmt.Errorf("input must be a CSV file (.csv), got %s", filepath.Ext(path))
	}
	return nil
}

...
class LouisVuittonScraper:
        ...
        try:
            ...
            for i, product_html in enumerate(individual_products):
                try:
                    ...
                    image_path = self.download_image_cdn(
    ...
    def download_image_cdn(self, image_url: str, product_name: str, image_folder: str, unique_id: str, retries: int = 3) -> str:
        """
        Download images from Louis Vuitton CDN with proper URL handling and headers
        """
        ...
        logger.info(f"[LV] Downloading image for: {product_name}")
        logger.info(f"[LV] Original Image URL: {image_url}")

        # Apply URL fixes
        fixed_image_url = self._fix_louisvuitton_url(image_url)
        logger.info(f"[LV] Fixed Image URL: {fixed_image_url}")

        # Enhanced headers for Louis Vuitton CDN
        headers = {
        ...
        for attempt in range(retries):
            try:
                ...
                logger.info(f"[LV] Attempt {attempt + 1}/{retries} for {product_name}")

                # First, try to get the main page to establish session
                if attempt == 0:
                    try:
                        session.get("https://eu.louisvuitton.com", timeout=10)
                        time.sleep(1)
                    except:
                        pass

                # Try the fixed URL first
                current_url = fixed_image_url
                ...
                # If 403, try alternative CDN parameters
                if response.status_code == 403:
                    logger.warning(f"[LV] 403 Forbidden on attempt {attempt + 1}")
                    
                    # Try alternative CDN parameters
                    base_url = fixed_image_url.split('?')[0]
                    alt_params = [
                        '?wid=720&hei=720',
                        '?wid=400&hei=400', 
                        '?fit=constrain,1&wid=800&hei=800',
                        '?wid=1440&hei=1440',
                    ]
                    
                    param_index = (attempt % len(alt_params))
                    current_url = base_url + alt_params[param_index]
                    logger.info(f"[LV] Trying alternative URL: {current_url}")
                    
                    response = session.get(current_url, timeout=30, allow_redirects=True, stream=True)

                # Check response
                if response.status_code != 200:
                    logger.warning(f"[LV] Status {response.status_code} on attempt {attempt + 1}")
                    continue

                # Verify content type
                content_type = response.headers.get('content-type', '')
                if not content_type.startswith('image/'):
                    logger.warning(f"[LV] Non-image content type: {content_type}")
                    continue
                ...
                if os.path.exists(image_full_path) and os.path.getsize(image_full_path) > 1000:  # At least 1KB
                    logger.info(f"[LV] ✅ Successfully downloaded {product_name} ({total_size} bytes)")
                    return image_full_path
                else:
                    logger.warning(f"[LV] Downloaded file too small or empty: {os.path.getsize(image_full_path)} bytes")
                ...
                logger.warning(f"[LV] Attempt {attempt + 1} failed: {str(e)}")
                continue
            except Exception as e:
                logger.warning(f"[LV] Attempt {attempt + 1} error: {str(e)}")
                continue

        logger.error(f"[LV] ❌ Failed to download {product_name} after {retries} attempts")
        return "N/A"

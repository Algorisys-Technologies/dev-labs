...
class MichaelHillParser:
    ...
    def make_michaelhill_image_high_quality(self, image_url: str) -> str:
        """Force best quality image for Michael Hill product URLs."""
        if not image_url:
            return image_url

        # 1. Separate base URL & query params
        if "?" in image_url:
            base, query = image_url.split("?", 1)
        else:
            base, query = image_url, ""

        # 2. Force high quality parameters
        params = {
            "sw": "1200",
            "sm": "fit",
            "q": "90"
    ...
    def download_image_sync(self, image_url: str, product_name: str, timestamp: str, image_folder_absolute: str, unique_id: str) -> str:
        ...
        try:
            ...
            # Generate filename
            image_filename = f"{unique_id}_{timestamp}{file_ext}"
            image_full_path = os.path.join(image_folder_absolute, image_filename)
            ...
            if response.status_code == 200:
                ...
                print(f"Image successfully saved: {image_full_path}")
                return image_full_path  # Return full absolute path
            else:

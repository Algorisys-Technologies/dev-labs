...
window.addEventListener('message', async (event) => {
  ...
  if (msg.type === 'selectorPicked') {
    ...
    // Fetch and display page information when scraping is done
    await fetchAndDisplayPageInfo();
  }
});
...
async function fetchAndDisplayPageInfo() {
  try {
    showStatus('Loading page information...', 'info');
    
    // Fetch page title and URL in parallel
    ...
    showStatus('Page information loaded', 'success');
    setTimeout(() => hideStatus(), 2000);
    
  } catch (error) {
    console.error('Error fetching page info:', error);
    pageTitleInput.value = "Error loading page title";
    pageUrlInput.value = "Error loading page URL";
...
function validatePageTitle() {
  ...
  if (!title) {
    showStatus('Please enter a page title before saving', 'warning');
    pageTitleInput.focus();
    return false;
  }
  
  if (title.length > 200) {
    showStatus('Page title is too long (max 200 characters)', 'warning');
    pageTitleInput.focus();
    return false;
  }
...
pageTitleInput.addEventListener('input', updateCharCount);

function updateCharCount() {
  const length = pageTitleInput.value.length;
  const maxLength = 200;
...
saveToDbBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    // Use the edited page title from input, or fall back to real page title
    const editedPageTitle = pageTitleInput.value.trim();
    const pageUrl = await getRealPageURL();

    console.log('Using page title:', editedPageTitle);
    const dataToSave = {
      page_title: editedPageTitle,
      pageUrl: pageUrl,
      products: currentScrapedData,
    ...
    const successMessage = `
      ✅ Successfully saved ${result.total_processed} products to database!
      📸 ${result.images_downloaded} images downloaded
      📝 Title: ${editedPageTitle.substring(0, 50)}${editedPageTitle.length > 50 ? '...' : ''}
    `.trim();
...
saveToDbBtn.title = 'Save to database (Ctrl+S)';
closeBtn.title = 'Close panel';
pageTitleInput.title = 'Edit the page title before saving (Ctrl+T to focus)';
pageUrlInput.title = 'Current page URL (read-only)';
